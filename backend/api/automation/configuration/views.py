...
from .serializers import ConfigurationEntrySerializer, ConfigurationSerializer

# Create your views here.


class ConfigurationEntryList(views.APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if user_id is not None:
            ...
            try:
                ...
                return Response(data={'detail': f"Invalid user id {user_id} for Entry"}, status=404)
...
class ConfigurationEntryDetail(views.APIView):
    queryset = Entry.objects.all()
    serializer_class = ConfigurationEntrySerializer
    permission_classes = (permissions.IsAuthenticated,)
    ...
    def get(self, request, pk, *args, **kwargs):
        try:
            ...
            return Response(data={'detail': f"Invalid id {pk} for Entry"}, status=404)
    ...
    def delete(self, request, pk, *args, **kwargs):
        try:
            entry_to_be_deleted: Entry = Entry.objects.get(
                pk=pk)
        except Entry.DoesNotExist:
            return Response(data={'detail': f"Invalid id {pk} for Entry"}, status=404)
